...
void Game::LoadAssetsAndCreateEntities()
{
	...
	// Describe and create our clamp state
	...
	device->CreateSamplerState(&clampDesc, clampSampOptions.GetAddressOf());
	...
	cobbleMat2xPBR->AddSampler("ClampSampler", clampSampOptions);
	...
	cobbleMat4xPBR->AddSampler("ClampSampler", clampSampOptions);
	...
	floorMatPBR->AddSampler("ClampSampler", clampSampOptions);
	...
	paintMatPBR->AddSampler("ClampSampler", clampSampOptions);
	...
	scratchedMatPBR->AddSampler("ClampSampler", clampSampOptions);
	...
	bronzeMatPBR->AddSampler("ClampSampler", clampSampOptions);
	...
	roughMatPBR->AddSampler("ClampSampler", clampSampOptions);
	...
	woodMatPBR->AddSampler("ClampSampler", clampSampOptions);
...
void Game::Update(float deltaTime, float totalTime)
{
	...
	{
		sky->computeShaderSkyShiftDay();
	}
	if (input.KeyDown(VK_LSHIFT))
	{
		sky->computeShaderSkyShiftNight();
	}
}
...
void Game::Draw(float deltaTime, float totalTime)
{
	...
	for (auto& ge : entities)
	{
		...
		ps->SetInt("SpecIBLTotalMipLevels", sky->GetMipMapLevels());

		ps->SetShaderResourceView("IrradianceIBLMap", sky->GetIrradianceCubeMap());
		ps->SetShaderResourceView("SpecularIBLMap", sky->GetConvolvedSpecularIBL_Cubemap());
		ps->SetShaderResourceView("BrdfLookUpMap", sky->GetBRDF_LookupTexture());
	...
	// - At the very end of the frame (after drawing *everything*)
	{
		...
		ImGui::Image(sky->GetBRDF_LookupTexture().Get(),ImVec2(500,500));
