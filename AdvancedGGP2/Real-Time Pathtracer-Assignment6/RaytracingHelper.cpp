...
void RaytracingHelper::CreateTopLevelAccelerationStructureForScene(std::vector<std::shared_ptr<GameEntity>> scene)
{
	...
	for (size_t i = 0; i < scene.size(); i++)
	{
		...
		entityData[meshBlasIndex].color[id.InstanceID] = XMFLOAT4(c.x, c.y, c.z, (float)(scene[i]->GetMaterial()->GetRoughness())); // Using alpha channel as "roughness" Setting Roughness
